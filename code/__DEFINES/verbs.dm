...
/proc/add_verb(client/target, verb_or_list_to_add, datum/callback/callback)
	...
	for(var/thing in verbs_list)
		...
		output_list[LIST_PRE_INC(output_list)] = list(verb_to_add.category, verb_to_add.name)
...
/proc/remove_verb(client/target, verb_or_list_to_remove)
	...
	for(var/thing in verbs_list)
		...
		output_list[LIST_PRE_INC(output_list)] = list(verb_to_remove.category, verb_to_remove.name)
